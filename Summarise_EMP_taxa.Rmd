...
biom_file <- "emp_cr_silva_16S_123.subset_2k.biom"
map_file <- "emp_qiime_mapping_subset_2k.tsv"
new_map_file <- "emp_qiime_mapping_subset_2k_noBLNK.csv"
Rank <- "Phylum"
Grouping_var <- "empo_3"
...
system("awk 'BEGIN { FS = OFS = \"\t\" } { for(i=1; i<=NF; i++) if($i ~ /^ *$/) $i = \"NA\" }; 1' emp_qiime_mapping_subset_2k.tsv  |  awk 'BEGIN { FS=\",\"; OFS=\";\" } {$1=$1; print}' |  awk 'BEGIN { FS=\"\t\"; OFS=\",\" } {$1=$1; print}' > emp_qiime_mapping_subset_2k_noBLNK.csv")
```
...
### Plot a heatmap
```{r plot heatmanp, fig.width=18, fig.height=25, cache=T}
# Agglomerate taxa
EMP_merge %>% 
